...
use MEASURED ABSENT — 2026-09-19 intervention (normal vs zeroed vs
shuffled vs replaced state at the query point) gives A=B=C=D=1/50, so
the state does not drive the output; exact task recall failed. Tidepool is currently a
...
language model. Update 2026-09-19: the failure is retrieval, not
retention — training builds slow channels itself (copy150k decays
min 0.77/mean 0.96), linear probes decode the payload at 10x chance
from slow-channel state, yet exact recall stays 0.0; fast-channel
state probes at 0%. Retention-side work is contraindicated; only
retrieval-side mechanisms (query-conditioned readout) remain. Update
2026-09-19b: delta-rule fast weights ARE retrieval-side and help FIT
(one-episode gate 2000 → 300 reps) but not the algorithm (recall
0/20 at 300k dense steps, query-CE plateaus above the no-fw run).
Signature of missing cross-step credit (RTRL traces cover RTU
states, not S). Next: trace extension to S, or stop. Update
2026-09-19c: episode-BPTT built and measured — memorization 2x2:
credit 20x, retrieval 7x, combined 40x (2000 → 50 episodes). But
generalization recall stays 0 at 700k tokens, and addressing never
rises above chance under any regime (everywhere-aux even poisons it
below chance). Retrieval work stops: credit helps fit, not algorithm.
...
## Continuation Value (2026-09-19)

The no-memory optimum on the copy probe is chance ((1/256)^4), so any
nonzero recall beats it — the threshold dissolves and the decision is
base-rate × cost. ~15 mechanism trials at 0 successes gives ≈6%
optimistic for the next distinct mechanism, and the single-episode
result reframes the gate: the main path cannot memorize ONE fixed
episode while a readout on frozen states fits 8/8, so every proposed
mechanism sits above a broken gate. Next is memorization surgery on
one episode (hours), not new memory (days); the frozen-state readout
fit is the under-celebrated positive — generation, not storage, is
the failing component. Update 2026-09-19: the gate is GREEN —
tests/test_memorize_one.py reproduces a fixed episode exactly via
free generation at 2000 reps (300 leaves the query transition at
rank 2). Prescription is now a copy-dense training run
