...
**Update 2026-09-20b: the external-store attempt is killed.** True
external KV store (write/erase/protect, content-only reads, learned
temperature, frozen trunk, BPTT): addressing 0.0875–0.10 vs 0.267
chance (~−3σ, queries point AWAY from payload slots), recall 0/20.
The line's central prediction failed its first fair test — below
chance, not at it. Terminal clause is live: recommended archival
absent a strategic decision otherwise. An en-route stale-graph bug
(K/V linking the freed prior graph) was fixed and pinned by test.
