...
## Transformer Anchor (Task Hard, Period)

Tiny causal transformer (462k params), offline batched multi-epoch:
loss 5.25 → 4.02 over 20 epochs, probe still 0.0. Even the reference
regime fails exact recall at small scale. The zeros are consistent
with task hardness, not RTU defect. Exact copy needs more than tried:
scale, training, or a softer bar.
...
## Input Gate (Prediction Yes, Recall No)

Selective + learned per-dim input write-gate (FD-proven), pure copy: 10k CE
8.262, 60k CE 6.973, probe 0.0 both. A dozen levers now move
prediction; zero move exact recall. Recall is not a knob problem at
this scale.

## Input Gate 60k (Identical to Without)

Selective+ingate at 60k reads payload CE 6.973 — identical to three
decimals to selective-without-ingate. Gates active (~0.03), FD-proven,
yet no measurable recall difference. Protection against overwriting
does not produce retrievability by itself.
...
## Constraint Reveals Fragments

Banning 0x00 at generation turns null cycles into repeating payload
fragments (oo@~oo@~Moo@~oo@) — still no exact match. The trap masked a
partial signal; sequencing fails by repetition, not degeneracy.
Diagnostic, not cure.

## Decode Ceiling (Distribution Lacks the Peak)

Presence-penalty sweep: fragments grow monotonically (pen 5 near-miss),
then pen 10/20 saturate identically with exotic bytes intruding;
sampled decoding 0/10. True bytes never top the distribution — tweaks
exhausted. Left: training that sharpens the peak, scale, structural
memory.

## Sparse Readout (Blur Upstream of Readout)

Top-16/128 readout sparsity, recurrence untouched: payload CE 7.462 vs
7.923 dense, probe still 0.0. Disentangling readout mixing helps
prediction slightly; the blur lives upstream in what gets written to
state, not in how it is read.

## Hybrid Store Fails (History Swamps Content)

Verbatim-store/softmax-retrieve hybrid: 0/20 copy, 0/10 assoc at both
lambdas on the trained model. Query states never resemble stored
states — raw recurrence is history-dominated, so neither exact nor
soft matching retrieves. Keying on bare embeddings would work
trivially and prove nothing. Content/context factorization is the
missing piece, and nothing tried learns it.

## Exact Cache (Wrong Shape for Recall)

Bounded K-gram table (4/4 unit tests green): copy probe 0/20 (query
boundary never observed continued — guaranteed opening miss), prose bpb
5.759 vs 5.750 (neutral). Exactness only continues verbatim
repetitions; kNN-LM works through similarity. Next honest design:
exact storage with soft similarity retrieval.

## LR Sweep on Copy (Prediction Knob, Not Recall)

Pure-copy 30k payload CE by LR: 1e-3 → 6.892, 3e-4 → 6.802, 1e-4 →
7.923; probe 0.0 at all three. Higher LR helps payload prediction,
recall untouched — LR moves prediction only, consistent with the whole
record.

## Fine LR Sweep (No Window With Recall)

10 LRs from 3e-5 to 1e-3 at 10k steps: probe 0.0 everywhere; CE best
(7.28) at the lowest LR, worsening monotonically upward. The literature
narrow window does not appear at this horizon (caveat: 10k steps may be
too few for any LR). Slowest is safest; nothing recalls.

## Long Low-LR (Window Fails Fair Test)

Best-LR 200k run: payload CE 7.036, probe 0.0, held-out flat ~7.0–7.4.
LR exhausted as a recall lever at this scale across 11 values and
10k–200k steps.
