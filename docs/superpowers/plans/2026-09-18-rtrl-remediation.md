...
- NOTE: `m(...)` here is inference-only (no backward); the torch persistent buffers update inside `forward`? They do NOT — buffers update in `_update` only. So the per-step buffer comparison must drive `_update`-equivalent buffer motion: call `m.training_step(curr, nxt, False)` inside `torch.no_grad()`? No — training_step runs backward + optimizer. Correct approach per implementer judgment: compare buffers after each `training_step` with `update_every=1000000` (no optimizer step) and `lr` irrelevant; gradients accumulate harmlessly. The test above uses `m(...)` for logits/states and must ALSO advance buffers: replace the `m(...)` call with a no-step `training_step` — keep logits/states assertions on its forward values? `training_step` returns (loss, sampled, stop), not states. Restructure: call `logits, states = m(...)` for the return-value assertions AND call the buffer-advancing path... that double-advances NumPy (once per mlx_step) vs torch buffers (advanced only by training_step). Cleanest: in the loop, FIRST `m.training_step(curr, nxt, False)` under `update_every=1000000` (buffers advance, no step), THEN read back `m.layers[i].states` etc. for buffer assertions and run `m(...)`? No — second forward doesn't change buffers (forward is pure) ✓. So: training_step (advances buffers) → with no_grad `logits, states = m(...)` (pure, same input) → compare all three against NumPy. Implementer: write the loop in exactly this order.
