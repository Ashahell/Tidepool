...
class InitialParallelRefine(ExplorationPolicy):
    ...
    def _refine(self, best: dict) -> dict:
        # Numeric keys return best clamped to the grid (pure exploitation;
        # numeric exploration rides on random_frac fresh samples).
        out = {}
    ...
    def select_next_configs(self, history: list[dict], budget: int) -> list[dict]:
        ...
        if not scored:
            # Fresh RNG per call: repeat calls are deterministic (tested).
